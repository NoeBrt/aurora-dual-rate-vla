...
## Cinematic Simulation Preview

The repo can generate visually rich synthetic runs with a pseudo-3D apartment scene, two mobile manipulators, clutter, drawer motion, wrist-camera inset, occupancy crop, sensor cones, trajectory glow, and safety telemetry.

![Cinematic simulated drawer run](docs/sim_runs/sim-drawer-tool-demo/contact_sheet.svg)

Generate a new cinematic run:
...
- `cinematic`: visually rich pseudo-3D scene with lighting, shadows, camera overlays, occupancy crop, sensor cones, and trajectory glow.
- `technical`: simpler top-down diagnostic renderer.
...
python3 scripts/simulate_aurora_runs.py --task drawer_tool --run-id sim-drawer-debug --frames 12 --style technical
```
